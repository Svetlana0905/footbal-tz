  ...
  return (
    <div className='leagues'>
      {props.currentPageArr.map(item =>
        <div key={item.id} className={props.nameSearchLeague === item.name ? 'leagues__block leagues__block--active' : 'leagues__block'}>
          <Tooltip
            title={arrCode.includes(item.code) ? 'Посмотреть календарь соревнований' : 'Извините, календарь недоступен'}
            color='#108ee9'
